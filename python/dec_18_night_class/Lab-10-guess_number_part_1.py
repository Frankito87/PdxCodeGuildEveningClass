...
def guess_number_part1():

    user_number = input("Enter a secret number between 1 and 100: ")
    print(user_number)

    guess_number = True

    while guess_number == True:
        if int(user_number) > 0 and int(user_number) <= 100:
            computer_guess = random.choice(numbers)
            print(computer_guess)

            if int(user_number) == int(computer_guess):

                print("You WIN !! You acert the number. Congratulations.")
                guess_number = False

            elif int(computer_guess) < int(user_number):
